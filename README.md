```markdown
<!-- PROFILE README -->

<h1 align="center">Tiago Henrique de Souza da Silva</h1>

<p align="center">
<img src="https://komarev.com/ghpvc/?username=tiagohenriquee&label=Profile%20views&color=0e75b6&style=flat" />
</p>

<p align="center">
Based in Tefé, Amazonas - Brazil
</p>

<p align="center">
<a href="mailto:tiagohenrique.dev@gmail.com">
<img src="https://img.shields.io/badge/Email-Contact-red?style=for-the-badge&logo=gmail">
</a>

<a href="https://github.com/tiagohenriquee">
<img src="https://img.shields.io/badge/GitHub-tiagohenriquee-black?style=for-the-badge&logo=github">
</a>

<a href="https://linkedin.com/in/tiagohenrique">
<img src="https://img.shields.io/badge/LinkedIn-Profile-blue?style=for-the-badge&logo=linkedin">
</a>
</p>

---

# About Me

I'm a **Full Stack Developer and Computer Engineering student** focused on building **scalable web systems and modern infrastructure environments**.

My work combines **software development and infrastructure engineering**, allowing me to design and deploy complete production-ready systems.

Currently working on **digital systems and platforms for the public sector**, where I develop and deploy applications used by municipal services.

My main interests include:

- System architecture
- DevOps and infrastructure automation
- Scalable backend systems
- Secure deployment environments

---

# Tech Stack

### Languages
![JavaScript](https://img.shields.io/badge/JavaScript-yellow?logo=javascript)
![TypeScript](https://img.shields.io/badge/TypeScript-blue?logo=typescript)

### Frontend
![React](https://img.shields.io/badge/React-20232A?logo=react)
![Vite](https://img.shields.io/badge/Vite-purple?logo=vite)
![HTML5](https://img.shields.io/badge/HTML5-orange?logo=html5)
![CSS3](https://img.shields.io/badge/CSS3-blue?logo=css3)

### Backend
![NodeJS](https://img.shields.io/badge/Node.js-green?logo=node.js)
![REST API](https://img.shields.io/badge/API-REST-black)

### DevOps / Infrastructure
![Docker](https://img.shields.io/badge/Docker-blue?logo=docker)
![Nginx](https://img.shields.io/badge/Nginx-green?logo=nginx)
![Linux](https://img.shields.io/badge/Linux-Ubuntu-orange?logo=ubuntu)
![Cloudflare](https://img.shields.io/badge/Cloudflare-Tunnel-orange?logo=cloudflare)

### Databases
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-blue?logo=postgresql)
![Redis](https://img.shields.io/badge/Redis-red?logo=redis)

---

# Featured Projects

## BolsaUni Tefé

A digital platform designed to manage **municipal university scholarships**.

Key features:

• Scholarship application system  
• Administrative management panel  
• Backend APIs for processing applications  
• Containerized architecture with Docker  
• Secure access through Cloudflare Tunnel  

Technologies used:

React • Node.js • PostgreSQL • Docker • Nginx

---

## Cloudflare Tunnel Infrastructure

Infrastructure project designed to securely publish internal services without requiring a public IP.

Key features:

• Secure exposure of services through Cloudflare Tunnel  
• Reverse proxy architecture with Nginx  
• Docker-based service orchestration  
• HTTPS and domain routing  

Technologies used:

Docker • Nginx • Cloudflare Tunnel • Linux

---

## Institutional Websites Standardization

Technical project focused on defining **standards for government institutional websites**.

Key contributions:

• Definition of UX/UI standards  
• Architecture guidelines for public sector websites  
• Standardization of layout and navigation  
• Collaboration between communication and technology departments  

---

# GitHub Stats

<p align="center">

<img height="160" src="https://github-readme-stats.vercel.app/api?username=SEU_USUARIO&show_icons=true&theme=tokyonight"/>

<img height="160" src="https://github-readme-stats.vercel.app/api/top-langs/?username=SEU_USUARIO&layout=compact&theme=tokyonight"/>

</p>

---

# Contribution Graph

<p align="center">
<img src="https://github-readme-activity-graph.vercel.app/graph?username=SEU_USUARIO&theme=tokyo-night">
</p>

---

# Currently Learning

- Advanced Docker orchestration
- Kubernetes fundamentals
- CI/CD pipelines
- Infrastructure as Code

---

# Contact

Email: tiagohenrique.dev@gmail.com

GitHub: https://github.com/tiagohenriquee

LinkedIn: https://linkedin.com/in/SEU_USUARIO

Location: Tefé, Amazonas - Brazil

---

⭐ If you like my projects, feel free to explore my repositories and connect.
```
